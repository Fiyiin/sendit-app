...
class Parcel {
  ...
  static async createParcel(req, res) {
    ...
    const values = [req.user.id, req.body.name, req.body.description, req.body.pickup, req.body.delivery, req.body.current_location, req.body.weight, req.body.receivers_name, req.body.receivers_no, req.body.status];
  ...
  static async getAllParcels(req, res) {
    ...
    try {
      const { rows, rowCount } = await _db2.default.query(findAllQuery, [req.user.id]);
  ...
  static async getSingleParcel(req, res) {
    ...
    try {
      const { rows } = await _db2.default.query(text, [req.params.id, req.user.id]);
      if (!rows[0]) {
  ...
  static async cancelParcel(req, res) {
    ...
    try {
      const { rows } = await _db2.default.query(findOneQuery, [req.params.id, req.user.id]);
      if (!rows[0]) {
        return res.status(404).json({ message: 'Parcel not found' });
      }
      const values = [req.body.status, req.params.id, req.user.id];
      const response = await _db2.default.query(updateOneQuery, values);
